...
export default function EventDetails() {
    ...
    error: deleteError } = useMutation({
      ...
      onSuccess: () => {
        queryClient.invalidateQueries({
          queryKey: ['events'],
          refetchType: 'none', // 기존 쿼리가 즉시 다시 트리거되지 않게 한다.
        });
        navigate('/events');
      }
    })

  function handleStartDelte() {
    setIsDeleting(true);
  }

  function handleStopDelte() {
    setIsDeleting(false);
  }
    ...
    content = (
      <>
        <header>
          <h1>{data.title}</h1>
          <nav>
            <button onClick={handleStartDelte}>Delete</button>
  ...
  return (
    <>
      {isDeleting && (
        <Modal onClose={handleStopDelte}>
          <h2>Are you sure?</h2>
          <p>Do you really want to delete this event? This action cannot be undone.</p>
          <div className='form-actions'>
            {isPendingDeletion && <p>Deleting, Please wait...</p>}
            {!isPendingDeletion && <>
              <button onClick={handleStopDelte} className='button-text'>Cancel</button>
